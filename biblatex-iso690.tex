...
entered \prm{locale} identifier. The default value is \t|auto|, meaning the
\prm{locale} is set to the main document language identifier (if
\t|babel|/\t|polyglossia| is used). A real use case example: this document is
in English (main document language), but uses Czech sorting
(\t|sortlocale=cs\_CZ|).
...
\begin{description}
  ...
  \item[url] enable or disable the printing of the URL. This option also
    affects the printing of \t|[online]| medium designation for electronic
    resources other than \t|@online| entry type (see \ref{sec:howpublished}).
...
The \t|biblatex-iso690| package provides new field \t|dateaddon| that can be
...
\item Replaced \t|clasification| with \t|number| field for patent
  entries\see{sec:patent}
